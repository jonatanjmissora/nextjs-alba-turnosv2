...
import { cn, getDayOfTheWeek } from "@/lib/utils";
...
const useMiniCalendar = () => {
    ...
    if (!context) {
        throw new Error(
            "MiniCalendar components must be used within MiniCalendar",
        );
    }
    ...
    return (
        <Button
            className={cn(
                ...
                isSelected &&
                    "bg-[#ff9bac]/50 shadow-[0px_0px_6px_0px_rgba(0,0,0,0.5)]",
                className,
            )}
        ...
        >
            <span
                className={cn(
                    "font-medium text-[10px] tracking-wider text-muted-foreground",
                )}
            >
                {month}
            </span>
